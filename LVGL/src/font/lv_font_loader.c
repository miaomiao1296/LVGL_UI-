...
#define lv_mem_free(ptr)  /* 什么都不做 */ ;
 extern void * lv_font_ccm_alloc(size_t size);
...
lv_font_t * lv_font_load(const char * font_name)
{
    ...
    lv_font_t * font = lv_font_ccm_alloc(sizeof(lv_font_t));
    if(font) {
    ...
    for(unsigned int i = 0; i < font_dsc->cmap_num; ++i) {
        ...
        switch(cmap_table[i].format_type) {
            case LV_FONT_FMT_TXT_CMAP_FORMAT0_FULL: {
                    ...
                    uint8_t * glyph_id_ofs_list = lv_font_ccm_alloc(ids_size);
            ...
            case LV_FONT_FMT_TXT_CMAP_SPARSE_TINY: {
                    ...
                    uint16_t * unicode_list = (uint16_t *)lv_font_ccm_alloc(list_size);
                    ...
                    if(cmap_table[i].format_type == LV_FONT_FMT_TXT_CMAP_SPARSE_FULL) {
                        uint16_t * buf = lv_font_ccm_alloc(sizeof(uint16_t) * cmap->list_length);
...
static int32_t load_cmaps(lv_fs_file_t * fp, lv_font_fmt_txt_dsc_t * font_dsc, uint32_t cmaps_start)
{
    ...
    lv_font_fmt_txt_cmap_t * cmaps =
        lv_font_ccm_alloc(cmaps_subtables_count * sizeof(lv_font_fmt_txt_cmap_t));
    ...
    cmap_table_bin_t * cmaps_tables = lv_font_ccm_alloc(sizeof(cmap_table_bin_t) * font_dsc->cmap_num);
                          ...
                          uint32_t start, uint32_t * glyph_offset, uint32_t loca_count, font_header_bin_t * header)
{
    ...
    lv_font_fmt_txt_glyph_dsc_t * glyph_dsc = (lv_font_fmt_txt_glyph_dsc_t *)
                                              lv_font_ccm_alloc(loca_count * sizeof(lv_font_fmt_txt_glyph_dsc_t));
    ...
    uint8_t * glyph_bmp = (uint8_t *)lv_font_ccm_alloc(sizeof(uint8_t) * cur_bmp_size);
...
static bool lvgl_load_font(lv_fs_file_t * fp, lv_font_t * font)
{
    lv_font_fmt_txt_dsc_t * font_dsc = (lv_font_fmt_txt_dsc_t *)
                                       lv_font_ccm_alloc(sizeof(lv_font_fmt_txt_dsc_t));
    ...
    bool failed = false;
    uint32_t * glyph_offset = lv_font_ccm_alloc(sizeof(uint32_t) * (loca_count + 1));
...
int32_t load_kern(lv_fs_file_t * fp, lv_font_fmt_txt_dsc_t * font_dsc, uint8_t format, uint32_t start)
{
    ...
    if(0 == kern_format_type) { /*sorted pairs*/
        lv_font_fmt_txt_kern_pair_t * kern_pair = lv_font_ccm_alloc(sizeof(lv_font_fmt_txt_kern_pair_t));
        ...
        uint8_t * glyph_ids = lv_font_ccm_alloc(ids_size);
        int8_t * values = lv_font_ccm_alloc(glyph_entries);
        ...
        lv_font_fmt_txt_kern_classes_t * kern_classes = lv_font_ccm_alloc(sizeof(lv_font_fmt_txt_kern_classes_t));
        ...
        uint8_t * kern_left = lv_font_ccm_alloc(kern_class_mapping_length);
        uint8_t * kern_right = lv_font_ccm_alloc(kern_class_mapping_length);
        int8_t * kern_values = lv_font_ccm_alloc(kern_values_length);
