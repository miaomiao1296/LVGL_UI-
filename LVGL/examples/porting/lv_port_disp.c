...
//    lv_disp_draw_buf_init(&draw_buf_dsc_1, buf_1, NULL, MY_DISP_HOR_RES * 10);   /*Initialize the display buffer*/
    ...
    /* Example for 2) */                       //占用SRAM：320*30*2*2/1024=37.5KB           
...
// 定义一个全局指针，好让中断知道通知哪个设备
 lv_disp_drv_t * disp_drv_ptr;
...
static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
   // 保存句柄，中断里要用
    ...
    // 2. 计算字节数
    uint32_t len = (uint32_t)(area->x2 - area->x1 + 1) * (area->y2 - area->y1 + 1) * 2;

    // 3. 调用异步发送
    LCD_Write_DMA_Async((uint8_t *)color_p, len);

    // 注意：这里删掉了 lv_disp_flush_ready！
    // 因为现在还没发完呢，不能跟 LVGL 说准备好了
}
